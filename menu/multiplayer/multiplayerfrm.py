...
class MultiplayerFrm(GameObject):
    ...
    def on_friend_answer(self, user, val):
        ...
        info('send presence subscription to %s: %s' % (
            user, 'subscribed' if val else 'unsubscribed'))
    ...
    def on_invite(self, usr):
        try:
            if not self.match_frm:
                self.eng.server.start(self.process_msg_srv,
                                      self.process_connection)
                self.create_match_frm('', True)
            self.match_frm.on_invite(usr)
        except (gaierror, URLError) as exc:
            gaierror_str = _(
            ...
            self.server_dlg = ServerDialog(self.menu_props, dct_msg[type(exc)])
    ...
    def on_yorg_init(self, msg):
        ...
        usr = [user for user in self.eng.xmpp.users
               if user.name == str(msg['from'].bare)][0]
    ...
    def on_presence_available(self, msg):  # unused msg
        self.users_frm.on_users()
    ...
    def on_room_back(self):
        ...
        if self.msg_frm.curr_match_room:  # if we've accepted the invitation
            ...
            if self.match_frm:
                # it's also invoked when server quits in car's page by
                # os_srv_quitted
                self.match_frm.destroy()
                self.match_frm = None
    ...
    def on_invite_chat(self, from_, to, roomname):  # unused to
