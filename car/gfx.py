...
class _Gfx(Gfx):
    ...
    def __init__(self, mdt, path):
        ...
        self.chassis_np = eng.loader.loadModel(path + '/car')
        self.chassis_np.reparentTo(self.nodepath)
        self.chassis_np.setDepthOffset(-2)

        try:
            self.front_right_wheel_np = eng.loader.loadModel(path + '/frontwheel')
            self.front_right_wheel_np.reparentTo(eng.world_np)

            self.front_left_wheel_np = eng.loader.loadModel(path + '/frontwheel')
            self.front_left_wheel_np.reparentTo(eng.world_np)

            self.rear_right_wheel_np = eng.loader.loadModel(path + '/rearwheel')
            self.rear_right_wheel_np.reparentTo(eng.world_np)

            self.rear_left_wheel_np = eng.loader.loadModel(path + '/rearwheel')
            self.rear_left_wheel_np.reparentTo(eng.world_np)
        except IOError:
            self.front_right_wheel_np = eng.loader.loadModel(path + '/wheel')
            self.front_right_wheel_np.reparentTo(eng.world_np)

            self.front_left_wheel_np = eng.loader.loadModel(path + '/wheel')
            self.front_left_wheel_np.reparentTo(eng.world_np)

            self.rear_right_wheel_np = eng.loader.loadModel(path + '/wheel')
            self.rear_right_wheel_np.reparentTo(eng.world_np)

            self.rear_left_wheel_np = eng.loader.loadModel(path + '/wheel')
            self.rear_left_wheel_np.reparentTo(eng.world_np)
